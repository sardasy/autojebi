...
from api.routers.notices import bid_pipeline, notice_spec_items
from api.services.hwp_agent_client import HwpAgentError, ProposalComposeOutcome
...
def test_hwp_compose_uses_spec_items_and_persists_exports(client, sqlite_engine, monkeypatch):
    ...
    class FakeClient:
        def autofill_bid_form(self, **kwargs):
            from api.services.hwp_agent_client import AutofillOutcome
    ...
    assert row.analysis["document_automation"]["exports"][0]["kind"] == "hwp"
...
def test_proposal_compose_persists_draft_and_export(client, sqlite_engine, monkeypatch, tmp_path):
    ...
    class FakeClient:
        def compose_proposal(self, **kwargs):
            assert kwargs["values"]["notice_no"] == "SPEC-1"
            assert "technical_compliance_summary" in kwargs["values"]
            assert kwargs["sections"][0]["title"] == "회사 소개"
            assert kwargs["tables"][0]["title"] == "규격 대응표"
            return ProposalComposeOutcome(
                output_path=str(final_path),
                replaced=["notice_no"],
                missing=[],
                remaining_placeholders=[],
                section_count=len(kwargs["sections"]),
                table_count=len(kwargs["tables"]),
                raw={},
            )
...
def test_proposal_compose_remaining_placeholders_marks_export_warning(
    ...
    assert body["errors"][-1]["stage"] == "proposal.placeholders"
...
def test_proposal_compose_agent_failure_keeps_draft_without_export(
    ...
    class FakeClient:
        def compose_proposal(self, **kwargs):
            raise HwpAgentError("agent missing /proposal/compose")
    ...
    assert any(item["stage"] == "proposal" for item in body["errors"])
    with sqlite_engine.begin() as conn:
        analysis = conn.execute(
            select(bid_pipeline.c.analysis).where(bid_pipeline.c.notice_no == "SPEC-1")
        ).scalar_one()
